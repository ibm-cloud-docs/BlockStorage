...
スナップショット・スケジュールを使用すると、ストレージ・ボリュームのポイント・イン・タイム参照を作成する頻度とタイミングを決定できます。1 つのストレージ・ボリュームにつき、最大 50 個のスナップショットを使用できます。
スケジュールの管理は、[{{site.data.keyword.slportal}}](https://control.softlayer.com/){:new_window}の**「ストレージ」**、「**{{site.data.keyword.blockstorageshort}}」**タブを介して行います。
