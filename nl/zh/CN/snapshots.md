...
{{site.data.keyword.blockstorageshort}} 提供了两种方法来生成快照。- 一种是通过可配置的快照安排来自动为每个存储卷创建和删除快照副本。您还可以根据需求来创建额外快照安排，手动删除副本和管理安排。 
- 另一种方法是生成手动快照。
...
您必须为卷购买一些快照空间量才能生成该卷的快照。在初始卷订购期间或订购后，可以通过**卷详细信息**添加快照空间。请注意，安排的快照和手动快照共享快照空间，因此请相应地订购空间。有关更多详细信息和指导信息，请参阅[订购快照](ordering-snapshots.html)文章。

## 快照最佳实践

快照设计取决于客户的环境。以下设计注意事项应该有助于您计划和实施快照副本： 
...
请记住，快照并不能替代实际的非现场灾难恢复复制或长期保留备份。
