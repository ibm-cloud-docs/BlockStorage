...
您必须先为卷购买一些快照空间量才能生成该卷的快照。在初始订购期间或之后，可以通过**卷详细信息**添加快照空间。安排的快照和手动快照共享快照空间，因此请确保订购足够的空间。有关更多详细信息和指导信息，请参阅[订购快照](ordering-snapshots.html)文章。
...
快照副本通过保留单个块而不是整个文件，使磁盘空间使用量尽可能最少。仅当更改或删除了活动文件系统中的文件时，快照副本才会使用额外的空间。发生这种情况时，原始文件块仍保留为一个或多个快照副本的一部分。
