...
通过快照安排，您可以决定创建存储卷的时间点引用的频率和时间。每个存储卷最多可以有 50 个快照。安排通过 [{{site.data.keyword.cloud}} 控制台](https://{DomainName}/classic){: external}中的**存储** > **{{site.data.keyword.blockstorageshort}}** 选项卡进行管理。
...
3. 单击**保存**。然后，您可以创建具有不同频率的其他安排。如果安排的快照总数超过 50 个，那么您将收到一条警告消息，并且无法保存该安排。

在生成快照时，会在**详细信息**页面的**{{site.data.keyword.blockstorageshort}}快照**部分中显示快照列表。
...
2. 单击**操作**。
3. 单击**生成手动快照**。
这将生成快照，并且快照会显示在**详细信息**页面的**{{site.data.keyword.blockstorageshort}}快照**部分中。其安排会显示为“手动”。
...
在**{{site.data.keyword.blockstorageshort}}详细信息**页面上，可以查看保留的快照和已使用空间量的列表。管理功能（编辑安排和添加更多空间）是在 **{{site.data.keyword.blockstorageshort}} 详细信息**页面上使用该页面上各部分中的**操作**菜单或链接来执行的。
...
**{{site.data.keyword.blockstorageshort}} 详细信息**页面上的饼图显示已使用的空间量以及剩余的空间量。在达到空间阈值（75%、90% 和 95%）时，您会收到通知。
...
1. 单击存储卷，单击**操作**，然后单击**添加快照空间**/**更改快照空间**。
...
达到三个不同的空间阈值（75%、90% 和 95%）时，会通过支持案例向帐户的主用户发送通知。

- 达到 **75% 容量**时，会发送警告来指示快照空间使用率已超过 75%。如果您留意到警告并手动添加空间或删除保留的不需要的快照，那么将记录相应操作并关闭该案例。如果不执行任何操作，那么必须手动确认该案例，然后该案例将关闭。
- 达到 **90% 容量**时，会发送第二个警告来指示快照空间使用率已超过 90%。与达到 75% 容量时一样，如果执行必要的操作来减少使用的空间，那么将记录相应操作并关闭该案例。如果不执行任何操作，那么必须手动确认该案例，然后该案例将关闭。
...
1. 单击 **{{site.data.keyword.blockstorageshort}} 详细信息**页面上**快照安排**框架中要删除的安排。
...
1. 从主机卸装并拆离存储卷。
   - [在 Linux 上连接 iSCSI 卷](/docs/infrastructure/BlockStorage?topic=BlockStorage-mountingLinux#unmounting)
   - [在 Microsoft Windows 上连接 iSCSI 卷](/docs/infrastructure/BlockStorage?topic=BlockStorage-mountingWindows#unmounting)
2. 单击 [{{site.data.keyword.cloud_notm}} 控制台](https://{DomainName}/){: external}中的**存储** > **{{site.data.keyword.blockstorageshort}}**。
3. 向下滚动并单击要复原的卷。**详细信息**页面的**{{site.data.keyword.blockstorageshort}}快照**部分将显示所有保存的快照及其大小和创建日期的列表。
...
5. 单击**是**以启动复原。
   ...
   {:note}
6. 安装存储卷并将其重新连接到主机。
   - [在 Linux 上连接 iSCSI 卷](/docs/infrastructure/BlockStorage?topic=BlockStorage-mountingLinux)
   - [在 CloudLinux 上连接 iSCSI 卷](/docs/infrastructure/BlockStorage?topic=BlockStorage-mountingCloudLinux)
   - [在 Microsoft Windows 上连接 iSCSI 卷](/docs/infrastructure/BlockStorage?topic=BlockStorage-mountingWindows)
