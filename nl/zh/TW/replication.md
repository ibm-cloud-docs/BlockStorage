...
## 在 {{site.data.keyword.blockstorageshort}} 清單中檢視抄本磁區

您可以在**儲存空間 > {{site.data.keyword.blockstorageshort}}** 的 {{site.data.keyword.blockstorageshort}} 頁面上檢視抄寫磁區。原始磁區和抄本磁區會分組在一起。**LUN 名稱**顯示主要磁區的名稱，其後接著 REP。**類型**為「耐久性或效能 - 抄本」。


## 在抄本資料中心檢視已抄寫磁區的詳細資料

檢視原始磁區的詳細資料時，您可以按一下**抄本**標籤，以便檢視抄本磁區詳細資料。另一個選項是從 **{{site.data.keyword.blockstorageshort}}** 清單選取抄本磁區，然後按一下**抄本**標籤。
...
如需相關資訊，請參閱[建立重複的 {{site.data.keyword.blockstorageshort}}](/docs/infrastructure/BlockStorage?topic=BlockStorage-duplicatevolume)。
...
取消主要磁區後，即會刪除抄寫排程及抄本資料中心內的磁區。抄本是從 {{site.data.keyword.blockstorageshort}} 頁面進行取消。
 ...
 1. 在 **{{site.data.keyword.blockstorageshort}}** 頁面上，按一下磁區名稱。
 2. 在 **{{site.data.keyword.blockstorageshort}} 詳細資料**頁面上，按一下**動作**，然後選取**取消抄本**。
 3. 選取何時取消。選擇**立即**或**週年日**，然後按一下**繼續**。
 4. 勾選方框，以確認您瞭解在取消磁區時可能發生資料流失。
 5. 按一下**取消抄本**。

 您可以預期 LUN 在「儲存空間」清單中會保持可見至少 24 小時（立即取消）或直到週年日為止。特定特性即將無法再使用，但磁區會保持可見直到收回為止。不過，在您按一下「刪除」/「取消抄本」之後，會立即停止計費。
