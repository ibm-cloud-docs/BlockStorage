...
Queste istruzioni sono relative a RHEL6/Centos6. Sono state aggiunte delle note per altri sistemi operativi ma questa documentazione **non** copre tutte le distribuzioni di Linux. Se stai utilizzando altri sistemi operativi Linux, fai riferimento alla documentazione della tua specifica distribuzione e assicurati che il multipath supporti ALUA per la priorità di percorso. 
...
2. Crea o modifica il file di configurazione multipath.
   - Modifica **/etc/multipath.conf** con la configurazione minima fornita nei seguenti comandi. <br /><br /> **Nota:** per RHEL7/CentOS7, `multipath.conf` può essere vuoto perché il sistema operativo ha delle configurazioni integrate. Ubuntu non utilizza multipath.conf poiché è integrato in multipath-tools.
...
6. Modifica le impostazioni CHAP in `/etc/iscsi/iscsid.conf` utilizzando il nome utente e la password dal {{site.data.keyword.slportal}}. Usa le maiuscole per i nomi CHAP.
   ...
   **Nota:** lascia le altre impostazioni CHAP come commenti. L'archiviazione {{site.data.keyword.BluSoftlayer_full}} utilizza solo un'autenticazione unidirezionale. Non abilitare Mutual CHAP
...
2. Crea una partizione sul disco.
   ...
   **Nota**: scorri ancora più in basso per i codici dei comandi elencati nella Tabella dei comandi `fdisk`.
...
2. Crea una partizione sul disco.
   ...
   4. Puoi usare `Parted` per creare partizioni disco primarie e logiche; la procedura prevista è la stessa. Per creare una partizione, `parted` utilizza `mkpart`. Puoi dargli dei parametri aggiuntivi come **primary** oppure **logical**, a seconda del tipo di partizione che vuoi creare.
   <br /> **Nota**: le unità elencate hanno come valore predefinito i megabyte (MB); per creare una partizione da 10 GB inizia da 1 e finisci a 10000. Puoi anche modificare le unità di dimensione in terabyte immettendo `(parted) unit TB`, se vuoi.
...
3. Crea un file system sulla nuova partizione.
   ...
   **Nota**: è importante selezionare il disco e la partizione corretti quando esegui questo comando!
   Verifica il risultato stampando la tabella partizioni. Nella colonna del file system, puoi vedere ext3.
...
Controlla che siano presenti i dischi. Conferma che ci sono due dischi con lo stesso identificativo e un elenco `/dev/mapper` della stessa dimensione con lo stesso identificativo. Il dispositivo `/dev/mapper` è il solo che viene configurato da multipath:
...
Se non è configurato correttamente, è simile al seguente esempio.
```
No multipath output root@server:~# multipath -l root@server:~#
```

Questo comando mostra i dispositivi inseriti nella blacklist.
```
