...
2. Controlla che siano presenti i dischi. Aspettati di vedere due dischi con lo stesso identificativo e un elenco `/dev/mapper` della stessa dimensione con lo stesso identificativo. Il dispositivo `/dev/mapper` è il solo che viene configurato da multipath.
  ```
  fdisk -l | grep Disk
  ```
  {: pre}
...
3. Se non disponi di altri volumi in quel portale di destinazione, elimina il record del portale di destinazione per impedire tentativi di login futuri.
   ...
   Per ulteriori informazioni, consulta il [manuale di `iscsiadm`](https://linux.die.net/man/8/iscsiadm).
   {:tip}
